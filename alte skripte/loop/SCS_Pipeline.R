...
setwd("C:/Users/Michael/Downloads/E-MTAB-11011p1")
...
GE = function(GeneList, pval = 1, byNames = FALSE, n = 200){
  ...
  if(byNames == TRUE){ #find gene enrichments by names only
    ...
    ngl = sort(ngl, decreasing = FALSE) #sort by increasing fold change value
...
# read files
hd = readRDS("pbmc.HD_gex_and_vdj.rds")
active = readRDS("pbmc.active.2.5.3.8_gex_and_vdj.rds")
recovered = readRDS("pbmc.recovered.14.16.26_gex_and_vdj.rds")
...
# GSEA for each cluster based on top 200 differentially expressed genes using GO 

# GeneList = active_clusters[[1]]
# GeneList = cbind(rownames(GeneList), GeneList) #append the row names (genes) as column
# colnames(GeneList)[1] = "Genename"
# 
# ngl = GeneList$p_val_adj #extract the adjusted p value
# names(ngl) = GeneList$Genename
# 
# ngl = sort(ngl, decreasing = FALSE) #sort by decreasing fold change value
# 
# num = ifelse(length(ngl) < 200, length(ngl), 200) #take the first n genes or all genes if num < n
# ngl = ngl[1:num] #retain the first n genes
# 
# #create topGOdata object
# GOdata = new("topGOdata",
#               ontology = "BP", #BP = Biological Process, MF = Molecular Functions, CC = 
#               allGenes = ngl,
#               topNodes = 10,
#               geneSel =function(x)x, #keep all genes
