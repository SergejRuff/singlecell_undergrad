...
scsPipeline = function(SeuratObj){
  ...
  SeuratObj = FindNeighbors(SeuratObj, dims = 1:10)#setzt keinen random.seed.
  SeuratObj = FindClusters(SeuratObj, resolution = 0.5,random.seed=runif(1))#setzt random.seed=0
  ...
  for(i in 1:length(clusterList)) clusterList[[i]] = FindMarkers(SeuratObj, ident.1 = as.numeric(levels(SeuratObj$seurat_clusters)[i]), min.pct = 0.25,random.seed=runif(1))
  #FindMarkers setzt random.seed=1 als Default 
...
setwd("A:/RFILES/bootstrap/17062022")
...
write.table(result,"test1Bootstraping170622.csv",quote=FALSE,sep=";")
