...
genes <- getBM(attributes = c("external_gene_name",  "entrezgene_id"),
               ...
               values = SeuratObj$Genename,
               mart=ensembl)

colnames(genes) = c("Genename", "entrezID")
Merged_data <-left_join(SeuratObj,genes_1 %>% dplyr::select(Genename, entrezID),by = "Genename")
head(Merged_data)
...
writexl::write_xlsx(kegg,'A:/RFILES/KEGG')
}
