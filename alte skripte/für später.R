...
setwd("A:/RFILES/bootstrap/19062022")
...
write.table(cluster_list,"rawdata3.csv",quote=FALSE,sep=";")
...
sort(table(boot$`rownames(cluster_list[[i]])`),decreasing =TRUE )
...
sort(table(boot$`rownames(cluster_list[[i]])`),decreasing =TRUE)
sort(table(unique(boot$`rownames(cluster_list[[i]])`)),decreasing =TRUE)
###############################################################################

#Tabelle auf Gene(V2)und p-val-adj (V1) reduzieren
abc=as.data.frame(cbind(boot$`rownames(cluster_list[[i]])`,boot.p_adj$p_val_adj))
#angeben, wie oft Gene signifikant aufgetaucht sind
sig.genes=sort(table(unique(abc)[,1]),decreasing = TRUE)
